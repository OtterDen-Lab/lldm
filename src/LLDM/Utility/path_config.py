...
class Routes:
    ...
    PATH_CONTEXT_BATTLE_SIMPLE_EVENT = os.path.join(RES_GPT_DIR, "BattleSimpleEvent.txt")
    PATH_CONTEXT_BATTLE_CONCLUDER = os.path.join(RES_GPT_DIR, "BattleConcluder.txt")
    PATH_CONTEXT_BATTLE_AI_EVENT = os.path.join(RES_GPT_DIR, "BattleAIEvent.txt")
    ...
    campaign = str(datetime.now().strftime("%m-%d-%Y (%I.%M.%S %p)"))  # Use time as unique campaign name
    ...
    BATTLE_CONTEXT_AI_EVENT = read(PATH_CONTEXT_BATTLE_AI_EVENT)
    BATTLE_CONTEXT_CONCLUDER = read(PATH_CONTEXT_BATTLE_CONCLUDER)
